...
const SYSTEM_PROMPT = [
  ...
  'Each finding MUST have: category, severity, confidence (number 0..1), agent_id, tool_name, trace_id, product_id (strings or null), title, summary (<=200 chars), recommendation, evidence_event_ids (array of integers referencing provided candidate event ids), requires_action (boolean).',
  'Duties:',
  '- Merge duplicate candidates that describe the same underlying issue into a single finding.',
  '- Assign severity and confidence based on evidence and context (trace, agent, tool, error).',
