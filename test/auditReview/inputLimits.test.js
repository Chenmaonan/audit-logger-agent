...
function normalizeForInsert(event) {
  return {
    ...
    entity_type: event.entity?.type ?? event.entity_type ?? null,
    entity_id: event.entity?.id ?? event.entity_id ?? null,
    llm_intent_json: event.llm_intent ? JSON.stringify(event.llm_intent) : null,
    error_message: null,
    tags: null,
