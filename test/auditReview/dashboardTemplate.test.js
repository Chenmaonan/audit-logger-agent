...
function assertNoMojibake(text) {
  assert.doesNotMatch(text, MOJIBAKE_RE);
}
...
test('renderDashboard renders download page actions as plain anchor links', () => {
  const html = renderDashboard({
    page: {
      title: '快照详情',
      page_actions: [
        { label: '下载 HTML', href: '/dashboard/agents/agent-1/snapshots/snap-1.html?download=1', kind: 'secondary', download: true },
      ],
    },
    sections: [{
      ...
      title: '快照信息',
      items: [{ label: 'Agent ID', value: 'agent-1' }],
    }],
  });

  assert.ok(html.includes('<a href="/dashboard/agents/agent-1/snapshots/snap-1.html?download=1" class="page-action secondary" download>下载 HTML</a>'));
  assert.equal(html.includes('fetch('), false);
  assert.equal(html.includes('<script'), false);
  assert.doesNotMatch(html, /Bearer|cookie|magic token|é|�|Ã|Â|鈥|涓|鎬|瀹/);
});


test('renderDashboard default chrome and empty state do not contain mojibake', () => {
  const html = renderDashboard({
    page: {
      updated_at: '2026-07-03T10:30:00.000Z',
      breadcrumbs: [
  ...
  assert.ok(html.includes('\u9875\u9762\u5bfc\u822a'));
  assert.ok(html.includes('\u6682\u65e0\u53ef\u5c55\u793a\u7684\u5ba1\u67e5\u6570\u636e'));
  assert.ok(html.includes('\u66f4\u65b0\u65f6\u95f4\uff1a2026-07-03T10:30:00.000Z'));
